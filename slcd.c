...
//			break;
//		case 0x01:
//			LCD->WF8B[LCD_Front_Pin[6]] = (LCD_C);  //TASK 2.5 - replace   with segment definition
//			LCD->WF8B[LCD_Front_Pin[7]] = (LCD_S_B | LCD_S_C  );  //TASK 2.5 - replace   with segment definition
//			break;
//		case 0x02:
//			LCD->WF8B[LCD_Front_Pin[6]] = (LCD_S_G  | LCD_S_E  | LCD_S_D  );  //TASK 2.5 - replace   with segment definition
//			LCD->WF8B[LCD_Front_Pin[7]] = (LCD_S_A  |LCD_S_B  );  //TASK 2.5 - replace   with segment definition
//			break;
//		case 0x03:
//			LCD->WF8B[LCD_Front_Pin[6]] = (LCD_S_G  | LCD_S_D  );  //TASK 2.5 - replace   with segment definition
//			LCD->WF8B[LCD_Front_Pin[7]] = (LCD_S_A  | LCD_S_B | LCD_S_C );  //TASK 2.5 - replace   with segment definition
//			break;
//		case 0x04:
 ...
 *----------------------------------------------------------------------------*/
void slcdDisplay(uint16_t value ,uint8_t format){	
	uint16_t tab[4];
	uint8_t i = 0;
	uint8_t j = 0;
	slcdClear();
...
void slcdznak(uint8_t ktory){	
	switch(ktory)
	{
		//kropka1
		case 1:
			LCD->WF8B[LCD_FRONT1]  |= LCD_S_DEC;
			break;
		//kropka2
		case 2:
			LCD->WF8B[LCD_FRONT3]  |= LCD_S_DEC;
			break;
		// colon
		case 3:	
			LCD->WF8B[LCD_FRONT7]  |= LCD_S_DEC;
			break;
		//kropka3
		case 4:
			LCD->WF8B[LCD_FRONT5]  |= LCD_S_DEC;
			break;
	}
}

void slcdCLRznak(void){	
LCD->WF8B[LCD_FRONT1] &= ~LCD_S_DEC;
LCD->WF8B[LCD_FRONT3] &= ~LCD_S_DEC;
LCD->WF8B[LCD_FRONT5] &= ~LCD_S_DEC;
LCD->WF8B[LCD_FRONT7] &= ~LCD_S_DEC;
